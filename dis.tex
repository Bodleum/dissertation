...
= 1\]
\[n_q \equiv 1 \ \text{(mod q)} \implies n_q = 1,\ q+1,\ 2q+1,\ \dots \quad
...
\begin{proof}
    ...
    Consider:
    \[x\varphi_k\varphi_l = (x^k)\varphi_l = {(x^k)}^l = x^{(kl)} = x\varphi_{kl}\]
    Because multiplication in the reals is commutative, \(\Aut{C_n}\) is abelian.
