...
\begin{enumerate}[\bfseries{Case} 1:]
    ...
    \item \( q \mid p - 1\).
        ...
        By Lemma \ref{lem:aut}, \(\Aut{C_p} \cong C_2\) and so a non-trivial
        homomorphism \(\varphi:C_q \mapsto \langle \beta:x \mapsto x^{-1}
        \rangle\).
        So if \(\langle x \rangle = C_p\) and \(\langle y \rangle = C_q\), we
        obtain:
        \[G = \langle \, x, y \mid x^p = y^q = 1,\ y^{-1}xy = x^{-1} \, \rangle\]
        % TODO: Classify presentation
\end{enumerate}
...
\begin{aligned}
    ...
    = x^{-1} \, \rangle \qquad &\text{if} \ q \mid p-1 \\
