...
The Class Equation, \(|G| = |\z{(G)}| + \sum^k_{i=1}|x_i^G|\), tells us \(|\z{(G)}|\) must be a multiple of \(p\) because both  \(|G|\) and \(|x^G|\) are multiples of \(p\). Hence \(|\z{(G)}| = p\).
...
% TODO: Classify groups of order pq
